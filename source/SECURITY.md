...
- Project skill installation requires Workspace Trust, confines writes to fixed per-project or per-user roots, and executes no provider scripts, package managers, hooks, or workspace code.
...
- Curated third-party extensions are referenced by fixed Marketplace IDs rather than repackaged; VS Code performs its normal Marketplace signature and publisher checks.
- Application-scoped built-in extensions are detected before Marketplace installation; CRUXIDE never intentionally downgrades a newer built-in extension.
