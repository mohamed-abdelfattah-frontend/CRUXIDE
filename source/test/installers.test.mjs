...
test('Windows separates CLI installation from GUI shortcut launch', async () => {
  ...
  assert.match(source, /\$ExtensionPackIds/);
...
test('macOS verifies checksum and installed extension ID', async () => {
  ...
  assert.match(source, /extension_pack_ids=/);
  ...
  assert.match(source, /is a built-in extension with version/);
  assert.match(source, /cannot be downgraded/);
  assert.match(source, /platform_provided_ids/);
  assert.doesNotMatch(source, /--install-extension "\$required_id" --force/);
});

test('both installers exactly mirror the 61-extension manifest', async () => {
  ...
  const manifestIds = JSON.parse(manifestText).extensionPack.map((id) => id.toLowerCase());
  const windowsBlock = windowsSource.match(/\$ExtensionPackIds\s*=\s*@\(([^]*?)\n\)/)?.[1];
  const macBlock = macSource.match(/extension_pack_ids=\(([^]*?)\n\)/)?.[1];
  assert.ok(windowsBlock);
  assert.ok(macBlock);
  const windowsIds = [...windowsBlock.matchAll(/'([^']+)'/g)].map((match) => match[1].toLowerCase());
  const macIds = [...macBlock.matchAll(/"([^"]+)"/g)].map((match) => match[1].toLowerCase());
  assert.deepEqual(windowsIds, manifestIds);
  assert.deepEqual(macIds, manifestIds);
});
