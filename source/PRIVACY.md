...
When the user explicitly confirms Setup or a Skills Manager installation, CRUXIDE copies selected bundled Markdown skills to `.crux` or `~/.cruxide`, writes agent adapter files and managed project-rule pointers, generates `PROJECT_RULES.md`, and records a local lock/config file. Project Local may write a private managed block to `.git/info/exclude`; it does not change the shared `.gitignore`. These local files are not transmitted to CRUXCODE.DEV. Provider-managed entries are not downloaded or executed.
