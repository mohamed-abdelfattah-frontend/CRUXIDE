...
The extension does not make background network requests. It reads only VS Code APIs needed to determine whether a workspace contains a configured code-file extension, and it asks VS Code to execute a small allowlist of actions selected by the user. The Home page can open the fixed CRUXCODE.DEV and founder LinkedIn HTTPS pages in the system browser only when their buttons are pressed.

When the user explicitly confirms Setup or a Skills Manager installation, CRUXIDE copies selected bundled Markdown skills to `.crux` or `~/.cruxide`, writes agent adapter files and managed project-rule pointers, generates `PROJECT_RULES.md`, and records a local lock/config file. It also writes `README.md` and `.gitignore` inside that folder, a Markdown guide per selected provider-managed entry under `external/`, and a timestamped copy of any CRUXIDE-owned file it replaces under `backups/`. Separately, CRUXIDE records one flag in VS Code's own extension storage the first time it offers Setup, so the offer is made once; that flag is stored before the user accepts or declines and contains no workspace data. Project Local may write a private managed block to `.git/info/exclude`; it does not change the shared `.gitignore`. These local files are not transmitted to CRUXCODE.DEV. Provider-managed entries are not downloaded or executed.
