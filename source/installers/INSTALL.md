...
Open PowerShell in the extracted `CRUXIDE-v1.0.1` folder. In File Explorer, open that folder, click the address bar, type `powershell`, and press Enter. Then run:
...
```powershell
Get-FileHash .\cruxide-1.0.1.vsix -Algorithm SHA256
```

macOS:

```bash
shasum -a 256 ./cruxide-1.0.1.vsix
```
